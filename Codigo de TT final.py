...
#url2 = 'http://192.168.137.63/640x480.jpg'  # Se cambia la direccion IP por la que genere la ESP32-CAM en automatico, no se pudo establecer una IP fija
#captura3 = cv2.VideoCapture(url2)

""" Envío de mensajes con librería requests """

#esp32ip = "192.168.137.63"  # Dirección IP de la ESP32-CAM
#url = f"http://{esp32ip}/tu_ruta"

#  Mensaje a enviar
mensaje = ( "BIENVENIDO, ESTO ES UN PROTOTIPO QUE PODRA APOYARTE EN TUS LANZAMIENTOS")
...
#print(response.text)
#time.sleep(15)  # Delay o retraso para que el jugador observe el mensaje con detenimiento

# Mensaje a enviar
#mensaje = ("AJUSTA TU BRAZO DE TIRO HASTA FORMAR UNA L O UN ANGULO RECTO")
# Parámetros de la solicitud GET
#params = {'message': mensaje}
#response = requests.get(url, params=params)  # Utilizar requests para enviar mensaje al url proporcionado
# Imprimir la respuesta del servidor
...
#  Mensaje a enviar
#mensaje = ( "ESTO ES UN ENTRENAMIENTO DE CATCH AND SHOOT, PREPARATE PARA RECIBIR EL PASE")
# Parámetros de la solicitud GET
#params = {'message': mensaje}
#response = requests.get(url, params=params) # Utilizar requests para enviar mensaje al url proporcionado
# Imprimir la respuesta del servidor
...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while True:
        ...
        try:
            ...
            if 90 < angle < 120 and stage == 'Posicion de tiro':
                stage = "Lanzo"
                counter += 1
                print(counter)
